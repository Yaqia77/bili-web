  ...
  <div class="commend-panel">
    ...
    <div class="video-list">
      <div
        ...
        :key="item?.videoId ?? idx"
      >
        <VideoItem :data="item" :layoutType="0"></VideoItem>
      </div>
    </div>
  </div>
  <VideoList></VideoList>
...
const resetCarouselWidth = () => {
  const screenWidth =
    document.documentElement.clientWidth - proxy.bodyPadding * 2;
  let ratio = 0.5;
  // 在更宽的屏幕上适当增大比例，避免过小
  if (screenWidth >= 1800) {
    ratio = 0.55;
    ...
    ratio = 0.5; // 中小屏保持与右侧协调
  }
  let width = Math.round(screenWidth * ratio);
